(function($){
	$.fn.dailog = function(options, callBack){
	    ...
	    var defaultDailog = {
	      ...
	      title:              '请先注册登录',                    //提醒信息
	      discription:        '为了您更好的体验,请先注册登录',          //描述
	      borderRadius:       '4px',                      //圆角
	      bottons:            ['确定','取消'],            //按钮信息
    ...
    $.showMsg = function (options) {
    	options = options || {};
		var init = function () {
			var msg =  typeof (options.msg)!='undefined' ? options.msg : '默认信息显示';
            var time =  typeof (options.time)!='undefined' ? options.time : 2000;
            if(timer !== null && typeof (timer)!='undefined') clearTimeout(timer)
			var layerDom = `<div class="showMsg" id="showMsg">${msg}</div>`;
